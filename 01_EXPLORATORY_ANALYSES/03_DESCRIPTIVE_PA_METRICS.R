...
list(data_named$wei,
  ...
  map(., ~ {
    ...
    ggsave(plot = p.hist.1, 
           filename = paste0("\\\\172.27.137.244\\vieillissement$\\Post doc & PhD\\Mathilde\\Analysis\\02_ARTICLE_2\\PCA-K-means-for-PA-features\\01_EXPLORATORY_ANALYSES\\plots\\p.hist.1_", sub(".* - ", "", names(.x)[2]), ".png"),
           height = 6, 
           width = 12)
    ggsave(plot = p.dens.1, 
           filename = paste0("\\\\172.27.137.244\\vieillissement$\\Post doc & PhD\\Mathilde\\Analysis\\02_ARTICLE_2\\PCA-K-means-for-PA-features\\01_EXPLORATORY_ANALYSES\\plots\\p.dens.1_", sub(".* - ", "", names(.x)[2]), ".png"),
           height = 6, 
           width = 12)
